...
from app.contracts.contracts import Flow
from app.models.models import Selector, Step, StepKind
from app.secrets import PLACEHOLDER_RE
...
# The viewport MUST match the one exploration and compilation ran at.
#
# Selectors are resolved against a 1280x720 page. Playwright's own default is
# 800x600, which on a responsive site collapses the navigation into a hamburger
# and hides the elements those selectors name -- so a generated suite failed on
# links that were merely off-screen, and the failure looked like a locator bug
# in the agent rather than a window size.
GENERATED_VIEWPORT = {"width": 1280, "height": 720}

CONFTEST = '''"""Pytest configuration for the generated flow tests.

Generated by the aivar orchestration agent. These are ordinary pytest files:
`pytest tests/generated` runs them with no part of the agent involved.
"""

import pytest
from playwright.sync_api import sync_playwright, Page

# Must match the viewport the agent explored and compiled selectors at. At a
# smaller size a responsive site collapses its navigation and the selectors
...
@pytest.fixture
def page() -> Page:
    """A browser page for one test, closed afterwards whatever happens."""
...
def write_suite(
    ...
    # Clear the previous run's suite first.
    #
    # This directory is the thing a person is told to run: `pytest
    # tests/generated`. Left to accumulate, it serves yesterday's tests for a
    # different application alongside today's, and they fail for reasons that
    # have nothing to do with the app under test. Each run's own copy is kept
    # beside its report, so nothing is lost by clearing here.
    for stale in out_path.glob("test_*.py"):
        try:
            stale.unlink()
        except OSError:
            logger.warning("could not remove stale test file %s", stale)
    ...
    # Rewrite conftest.py every time rather than only when absent, so a fix here
    # reaches suites generated by an older version.
